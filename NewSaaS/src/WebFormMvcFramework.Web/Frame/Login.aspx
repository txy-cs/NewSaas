<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Login.aspx.cs" Inherits="WebFormMvcFramework.Web.Frame.Login" %>
...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            $.post('../Frame/Frame.ashx', { "action": "init" }, function (data) {
            })
        })
        //回车键
        ...
        function CheckUserDataValid() {
            if (!LoginBtn()) {
                ...
                CheckingLogin(1);
                $.post('../Frame/Frame.ashx', { "action": "login", "user_Account": escape(userName), "userPwd": escape(userPwd), "code": escape(code) }, function (rs) {
                    if (parseInt(rs) == 1) {
                        $("#txtCode").focus();
...
<body style="padding-top: 167px">
    <form id="form1" runat="server">
        <div class="boxLogin">
            ...
            <div class="s8">
                <input id="Log_Submit" type="button" class="sign" onclick="CheckUserDataValid()" />
            </div>
        </div>
        <div class="copyright">
            <p id="cp">
                最佳浏览环境：IE8.0～11.0或基于IE内核的浏览器，1280×800显示分辨率。
            </p>
        </div>
    </form>
</body>
