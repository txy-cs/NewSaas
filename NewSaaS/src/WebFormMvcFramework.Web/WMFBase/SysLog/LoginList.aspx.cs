...
namespace WebFormMvcFramework.Web.WMFBase.SysLog
{
    public class LoginList : PageBase
    {
        ...
        private void DataBindGrid()
        {
            ...
            SqlWhere.Append("SELECT Sys_LoginLog_ID,Sys_LoginLog_IP,SYS_LOGINLOG_TIME,User_Account,Sys_LoginLog_Status,OWNER_address from Base_SysLoginlog where 1=1");
            if (this.BeginBuilTime.Value != "" || this.endBuilTime.Value != "")
            {
                SqlWhere.Append(" and Sys_LoginLog_Time >= '" + CommonHelper.GetDateTime(this.BeginBuilTime.Value + "'"));
                SqlWhere.Append(" and Sys_LoginLog_Time <= '"+CommonHelper.GetDateTime(this.endBuilTime.Value).AddDays(1.0)+"'");
            }
            if (this.txt_Search.Value != "")
            {
                SqlWhere.Append(" and User_Account = '" + this.txt_Search.Value + "'");
            }
            SqlWhere.Append(" ORDER BY SYS_LOGINLOG_TIME DESC ");
            ...
            TB = Universal_ClassLibrary.DataTable.DataTableSort.DataTableSmartSort(TB.Copy(), "SYS_LOGINLOG_TIME", Universal_ClassLibrary.LiftingType.DESC).Copy();
        ...
        protected void rp_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (lbl_Sys_LoginLog != null)
                {
                    ...
                    text = text.Replace("1", "<span style='color:Blue'>成功登陆</span>"); // LoginLog_Status=1, 成功登陆；LoginLog_Status=8，退出登录
