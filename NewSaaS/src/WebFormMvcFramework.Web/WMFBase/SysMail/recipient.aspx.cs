...
{
    public partial class recipient : PageBase
    {
        ...
        private void DataBindGrid()
        {
            ...
            SqlWhere.Append("  where(Mail_User_Account='" + RequestSession.GetSessionUser().UserAccount.ToString() + "' and Mail_DeleteMark=1 and Mail_Type=1)  ");
        ...
        protected void lbut_delete_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < HF_KEY.Value.Split(',').Length; i++)
            {
                user_mail.DeleteMail(HF_KEY.Value.Split(',')[i]);
            }
            this.DataBindGrid();
            ShowMsgHelper.Alert("邮件删除成功！");
        }
    }
}