...
namespace WebFormMvcFramework.Web.WMFBase.SysUser
{
    public class UserInfo_Form : PageBase
    {
        ...
        {
            Hashtable ht = system_idao.GetHashtableById("Base_UserInfo", "User_ID", this._key);
        ...
        {
            DataTable dtRole = this.system_idao.InitRoleList();
        ...
        protected void Save_Click(object sender, EventArgs e)
        {
            ...
            bool IsOk = system_idao.Submit_AddOrEdit("Base_UserInfo", "User_ID", this._key, ht);
            if (IsOk)
            {
                ...
                for (int i = 0; i < item_value.Length; i++)
                {
                    ...
                    if (item.Length > 0)
                    {
                        ...
                        if (type == "所属部门")
                        {
                            ...
                            sbadd.Append(")Values(");
                            sbadd.Append("'" + CommonHelper.GetGuid + "','" + key + "','" + user_id + "','" + RequestSession.GetSessionUser().UserId + "','" + RequestSession.GetSessionUser().UserName + "')");
                        ...
                        else
                        {
                            if (type == "所属角色")
                            {
                                ...
                                sbadd.Append(")Values(");
                                sbadd.Append("'" + CommonHelper.GetGuid + "','" + user_id + "','" + key + "','" + RequestSession.GetSessionUser().UserId + "','" + RequestSession.GetSessionUser().UserName + "')");
                                sqls.Add(sbadd.ToString());
                            }
                            else
                            {
